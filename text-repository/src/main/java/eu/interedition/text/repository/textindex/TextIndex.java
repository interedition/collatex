...
public class TextIndex implements InitializingBean, DisposableBean {
  ...
  public List<TextIndexQueryResult> query(TextIndexQuery query) throws IOException, ParseException {
  ...
  public void update(Text text) throws IOException {
    ...
    textRepository.read(text, new TextRepository.TextReader() {
      ...
      public void read(Reader content, long contentLength) throws IOException {
        ...
        document.add(new Field("content", content, Field.TermVector.YES));
      ...
      public void run() {
        new TransactionTemplate(transactionManager).execute(new TransactionCallbackWithoutResult() {
          ...
          protected void doInTransactionWithoutResult(TransactionStatus status) {
            try {
              if (jt.queryForInt("select count(*) from text_content") == 0) {
                indexWriter.deleteAll();
              } else if (indexWriter.numDocs() == 0) {
