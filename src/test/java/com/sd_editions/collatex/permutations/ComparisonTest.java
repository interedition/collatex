...
public class ComparisonTest extends TestCase {
  ...
  public void testAddition_InTheMiddle() {
    final Modifications modifications = getModifications("a cat", "a calico cat");
  ...
  public void testReplacementVariantAtTheStart() {
    final Modifications modifications = getModifications("black cat", "white cat");
    assertEquals(1, modifications.size());
    assertEquals("replacement: black / white position: 1", modifications.get(0).toString());
  }

  public void testReplacementVariantAtTheEnd() {
    final Modifications modifications = getModifications("it's black", "it's white");
    assertEquals(1, modifications.size());
  ...
  public void testPhraseVariantReplacementAtTheStart() {
    final Modifications modifications = getModifications("a b c d", "e f g c d");
    assertEquals(1, modifications.size());
  ...
  public void testPhraseVariantReplacementAtTheEnd() {
    final Modifications modifications = getModifications("a b c d", "a b e f g");
    assertEquals(1, modifications.size());
  ...
  public void testCombineAdditionAndRemovalToTestPositions() {
    final Modifications modifications = getModifications("a b c", "c d e");
  ...
  public void testCombineAdditionAndRemovalToTestPositionsMirrored() {
    final Modifications modifications = getModifications("c d e", "a b c");
    assertEquals(2, modifications.size());
  ...
  private Modifications getModifications(final String base, final String witness) {
    final Modifications permutations = new CollateCore(builder.build(base), builder.build(witness)).compareWitness(1, 2);
    return permutations;
  }
