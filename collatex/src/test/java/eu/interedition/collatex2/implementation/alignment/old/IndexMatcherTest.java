...
package eu.interedition.collatex2.implementation.alignment.old;
...
public class IndexMatcherTest {
	...
	// final IWitness witnessB = factory.createWitness("B",
	// "everything is unique");
	// final IAlignment alignment = PairwiseAlignmentHelper.align(factory,
	// witnessA, witnessB);
	...
	// final IWitness witnessC = factory.createWitness("C",
	// "everything is different");
	// final IAlignmentTable table = factory.align(witnessA, witnessB);
	// final List<ITokenMatch> matches = factory.alignOldStyle(table,
	// witnessC).getTokenMatches();
	...
	// }
	//
	// //Note: internally this gives # the big black and the big black cat as
	// matches
	// @Test
	...
	// }
	//
	// //TODO: make convenience method for creation of AlignmentTable on
	// Factory!
	//
	// @Test
	...
	// final IWitness c = factory.createWitness("C",
	...
	private void assertContains(final List<Tuple<List<INormalizedToken>>> matches,
		...
		final Iterable<String> normalizedMatches = Iterables.transform(matches,
				function);
		assertTrue(string + " not found in matches: "
				+ Joiner.on(",").join(normalizedMatches), Lists.newArrayList(
				normalizedMatches).contains(string));
	}
