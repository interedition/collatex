...
public class DeTestDirkVincent {
  ...
  public void testDirkVincent5() {
    IVariantGraph graph = factory.graph(factory.createWitness("01b", "Its soft light neither daylight nor moonlight nor starlight nor any light he could remember from the days & nights when day followed night & vice versa."));
    Iterator<IVariantGraphVertex> iterator = graph.iterator();
  ...
  public void testDirkVincent6() {
    IVariantGraph graph = factory.graph();
    graph.add(factory.createWitness("01b", "Its soft light neither daylight nor moonlight nor starlight nor any light he could remember from the days & nights when day followed night & vice versa."));
    graph.add(factory.createWitness("10a", "Its soft changeless light unlike any light he could remember from the days and nights when day followed hard on night and vice versa."));
  ...
  public void testDirkVincent7() {
    ...
    IVariantGraph graph = factory.graph(listener);
    graph.add(factory.createWitness("01b", "Its soft light neither daylight nor moonlight nor starlight nor any light he could remember from the days & nights when day followed night & vice versa."));
    graph.add(factory.createWitness("10a", "Its soft changeless light unlike any light he could remember from the days and nights when day followed hard on night and vice versa."));

    graph.add(factory.createWitness("dummy", "bla"));
    final Iterator<INormalizedToken> tokenIterator = listener.getSuperbase().tokenIterator();
  ...
  public void testDirkVincent8() {
    final MemoryzingVariantGraphListener listener = new MemoryzingVariantGraphListener();
    final IVariantGraph graph = factory.graph(listener);
    graph.add(factory.createWitness("01b", "Its soft light neither daylight nor moonlight nor starlight nor any light he could remember from the days & nights when day followed night & vice versa."));
    graph.add(factory.createWitness("10a", "Its soft changeless light unlike any light he could remember from the days and nights when day followed hard on night and vice versa."));
    graph.add(factory.createWitness("dummy", "bla"));
    ...
    IMatchResult result = analyzer.analyze(listener.getSuperbase(), c);
  ...
  public void testDirkVincent10() {
    ...
    graph.add(factory.createWitness("01b", "Its soft light neither daylight nor moonlight nor starlight nor any light he could remember from the days & nights when day followed night & vice versa."));
    graph.add(factory.createWitness("10a", "Its soft changeless light unlike any light he could remember from the days and nights when day followed hard on night and vice versa."));
    graph.add(factory.createWitness("11", "Its faint unchanging light unlike any light he could remember from the days & nights when day followed on night & night on day."));
...
//    assertEquals("TokenSequence: #: 0, he: 3, ", iterator.next().toString());
  ...
  public void testSentence42Transposition() {
    ...
    graph.add(factory.createWitness("06-1", "The same clock as when for example Magee once died."));
    graph.add(factory.createWitness("06-2", "The same as when for example Magee once died."));
    checkGraph(graph, "the", "same", "clock", "as", "when", "for", "example", "magee", "once", "died", ".");

    graph.add(factory.createWitness("08-01", "The same as when for example McKee once died ."));
    checkGraph(graph, "the", "same", "clock", "as", "when", "for", "example", "magee", "mckee", "once", "died", ".");

    graph.add(factory.createWitness("08-02", "The same as when among others Darly once died & left him."));
    checkGraph(graph, "the", "same", "clock", "as", "when", "for", "example", "magee", "mckee", "among", "others", "darly", "once", "died", "&", "left", "him", ".");

    graph.add(factory.createWitness("xxx", "The same as when Darly among others once died and left him."));

    final IAnalysis analysis = listener.getAnalysis();
