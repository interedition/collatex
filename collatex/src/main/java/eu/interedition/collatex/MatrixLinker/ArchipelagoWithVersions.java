...
public class ArchipelagoWithVersions extends Archipelago {
	...
	public ArchipelagoWithVersions(MatchMatrixIsland isl) {
		islands = new ArrayList<MatchMatrixIsland>();
		nonConflVersions = new ArrayList<Archipelago>();
		add(isl);
  }
  			...
  			if(!found_one) {
    			...
    			for(Archipelago version : nonConflVersions)	{
    				if(debug)
    				  System.out.println("loop 2: "+tel_loop++);
						MatchMatrixIsland island_copy = island.copy();
  					for(MatchMatrixIsland isl : version.iterator()) {
    			...
    			for(Archipelago version : nonConflVersions)	{
    				if(debug)
  					...
  					for(MatchMatrixIsland isl : version.iterator()) {
  						MatchMatrixIsland di = isl.copy();
  		  			if(debug)
  		  				System.out.println("di: "+di);
  						MatchMatrixIsland res = di.removePoints(island);
  		  			if(debug)
...
//					System.out.println("confl: "+isl+" with: "+i+" : "+result.get(i));
	...
	private double distance(MatchMatrixIsland isl1, MatchMatrixIsland isl2) {
		double result = 0.0;
	...
	public String createXML(MatchMatrix mat, PrintWriter output) {
  	...
  	list.add(new MatchMatrixCell(colNum-1, rowNum-1));
...
//  	System.out.println("3");
  	ArrayList<MatchMatrixCell> gaps = createFirstVersion.findGaps(list);
  	System.out.println("4");
  	ArrayList<Integer[]> listOrderedByCol = new ArrayList<Integer[]>();
  	int teller = 0;
  	Integer[] isl = {0,0,0,0,0};
  	for(MatchMatrixCell c : gaps) {
			teller++;
			if(teller%2==0) {
			  isl[0] = teller/2;
			  isl[3] = c.col;
  	...
  	int gapsPos = 0;
		...
		teller = 0;
		MatchMatrixCell vorige = new MatchMatrixCell(0,0);
		for(MatchMatrixCell c : gaps) {
			teller++;
			if(teller%2==0) {
				System.out.print(" " + c + " ");
